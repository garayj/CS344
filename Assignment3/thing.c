...
{ //Code to catch and display message for signals
  char *message = "\nCaught SIGINT.\n";
...
{ //All the variables
  ...
  while (1)
  {
    ...
    {
      if (strcmp(token, "<") == 0)
      {                              //This means input file
        token = strtok(NULL, " \n"); //Parse and store
        inputFile = strdup(token);
        token = strtok(NULL, " \n");
      }
      else if (strcmp(token, ">") == 0)
      { //This is an output file so perform same action and save to outputFile
        token = strtok(NULL, " \n");
    ...
    { //If no input or pound sign indicating comment, do nothing and move on
      ;
    }
    else if (strcmp(argument[0], "cd") == 0)
    { //User wants to change directories. If there is no argument after cd command, go to home directory
      if (argument[1] == NULL)
      { //Else, go to directory stated by user
        chdir(getenv("HOME"));
      }
      else
      {
    ...
    else
    {
      ...
      case 0: //Fork a new process and run switch on pid(idea taken from lecture)
        if (fgProcess)
        { //Foreground can be interrupted if need be
          action.sa_handler = SIG_DFL;
          sigaction(SIGINT, &action, NULL);
        }
        if (inputFile != NULL)
        { //If unable to open and read file, inform user
        ...
        if (outputFile != NULL)
        { //Inform user if output file cannot be opened
        ...
        { //Command is not recognized so inform the user and change exit status
      ...
      default:
        if (fgProcess)
        { //Wait for foreground to finish (that's why we pass specific cpid)
